@section("meta-title")
<title>Indriya | BlogCategory</title>
@endsection
@section("stylesheets")
<style>
    .switch_box {
                ...
                <div class="row">
                    <div class="col-md-12">
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <label class="switch_box">
                                                        <input type="checkbox" class="checkbox" data-id="{{$blog_category->id}}" {{($blog_category->status==1)?"checked":""}}>
                                                        <span class="slider round"></span>
                                                    </label>
                                                </td>
                                                <td>
                                                    <button class="btn btn-primary"
                                                        wire:click="editBlogCategory({{ $blog_category->id }})"><i
                                                            class="far fa-edit"></i></button>
                                                    <button class="btn btn-danger delete"
                                                        data-value="{{ $blog_category->id }}"><i
                                                            class="fas fa-trash-alt"></i></button>
                                                </td>
                                            </tr>
                                        @empty
                                            <tr>
                                                <td colspan="4" class="text-center">No BlogCategories</td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                                </table>
                        ...
                        <ul class="pagination float-right">
                            <li class="page-item">{{ $blog_categories->links() }}</li>
                        </ul>
                    </div>
                </div>
            </div>
    ...
    <div wire:ignore.self class="modal fade" id="modal-lg">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <form wire:submit="addBlogCategory" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="modal-footer float-right">
                        <button type="button" wire:click="addBlogCategory" class="btn btn-primary">Submit</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div wire:ignore.self class="modal fade" id="edit_modal_form" class="edit_model">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <form wire:submit="updateBlogCategory">
                    @csrf
                    ...
                    <div class="modal-footer float-right">
                        <button type="button" wire:click="updateBlogCategory" class="btn btn-primary">Submit</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        ...
        Livewire.on('delete', function(data) {
            if (data.parameter == "200") {
                Swal.fire({
                    title: "Deleted!",
                    text: "BlogCategory has been deleted.",
                    icon: "success"
                });
            } else {
